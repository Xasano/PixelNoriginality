...
export const VisitorBanner: React.FC = () => {
    ...
    const fetchVisitorLimits = async () => {
        try {
            setLoading(true);
            const response = await fetch("http://localhost:8000/api/visitors/limits", {
                credentials: "include"
            });
            ...
            if (data.success) {
                ...
                console.error("Erreur lors de la récupération des limites:", data.message);
            }
        } catch (error) {
    ...
    const isNearDailyLimit = limits?.pixelsRemaining !== undefined && limits.pixelsRemaining <= 5;
    // Détermine si on a atteint la limite quotidienne
    const hasReachedDailyLimit = limits?.pixelsRemaining !== undefined && limits.pixelsRemaining <= 0;
    // Détermine si on est en attente pour placer un nouveau pixel
    const isWaitingForNextPixel = limits?.timeUntilNextPixel !== undefined && limits.timeUntilNextPixel > 0;

    // Couleur de la bannière selon l'état
    const getBannerColorClass = () => {
        if (hasReachedDailyLimit) return 'bg-red-700 dark:bg-red-900 border-red-600 dark:border-red-800';
        if (isNearDailyLimit) return 'bg-orange-700 dark:bg-orange-900 border-orange-600 dark:border-orange-800';
    ...
    const getCompactStatusText = () => {
        ...
        if (isWaitingForNextPixel) return `Prochain: ${formatTime(limits?.timeUntilNextPixel || 0)}`;
    ...
    return (
        <>
            ...
            {/* Bannière détaillée */}
            {showBanner && limits && (
                <div className={`fixed bottom-0 left-0 right-0 p-3 shadow-lg z-30 border-t text-white
                    ${getBannerColorClass()}`}>
                    <div className="container mx-auto">
                            ...
                            {!hasReachedDailyLimit && (
                                <div className="text-sm flex flex-col sm:flex-row sm:items-center">
                                    <div className="sm:mr-4">
                                        <span className="font-semibold">{limits.pixelsRemaining}</span> pixels restants aujourd'hui
                                    </div>
                                    {isWaitingForNextPixel && (
                                        <div>
                                            Prochain pixel dans <span className="font-semibold">{formatTime(limits.timeUntilNextPixel)}</span>
                                        </div>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="flex justify-between items-center">
                            <div className="text-xs">
                                ...
                                <span>Réinitialisation dans {formatTime(limits.timeUntilDailyReset)}</span>
                            </div>
                            <a
                                href="/register"
