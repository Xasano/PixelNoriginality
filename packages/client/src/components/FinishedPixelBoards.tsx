...
const FinishedPixelBoards3DCarousel = () => {
    ...
    const fetchCompletedPixelBoards = async () => {
        try {
            ...
            const now = new Date();
            const completedBoards = response.data.data.filter(board => {
                return board.status === 'completed' || (board.status === 'active' && new Date(board.endDate) <= now);
            });

            setPixelBoards(completedBoards);
            setError(null);

            if (completedBoards.length > 0 && currentIndex >= completedBoards.length) {
                setCurrentIndex(0);
            }
        } catch (err) {
    ...
    return (
        <div className="space-y-6 relative">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h2 className="font-bold text-2xl mb-4 text-gray-800 dark:text-white">PixelBoards terminés</h2>
                ...
                {pixelBoards.length > 0 && !loading && (
                    <span className="px-3 py-1 bg-purple-100 text-purple-700 border border-purple-200 rounded-full text-sm font-medium">
                        {pixelBoards.length} PixelBoard{pixelBoards.length > 1 ? 's' : ''} terminé{pixelBoards.length > 1 ? 's' : ''}
                    </span>
                )}
            </div>
            ...
            {pixelBoards.length === 0 && !loading ? (
                <div className="text-center py-12 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-700 mb-2 font-medium">Aucun PixelBoard terminé</p>
                        <p className="text-sm text-gray-500">Participez à des PixelBoards actifs pour les voir ici une fois terminés</p>
                    </div>
                </div>
            ) : (
                <div className="relative">
                    ...
                    <div className="carousel-container overflow-hidden relative h-[450px] py-8 mx-auto">
                        <div className="carousel-3d-container relative w-full h-full">
                            {pixelBoards.length > 0 && visibleSlides.map(({ index, position }) => {
            ...
            {loading && (
                <div className="flex flex-col items-center justify-center py-10">
                    ...
                    <p className="text-gray-500 text-sm mt-4">Chargement des PixelBoards terminés...</p>
                </div>
            )}
