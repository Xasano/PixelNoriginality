...
const UserAccountDetails: React.FC<UserAccountDetailsProps> = ({ currentUser, formatDate }) => {
  return (
    <div>
      ...
      <ul className="space-y-3 bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
        ...
        <li className="flex flex-col">
        <span className="text-sm text-gray-500 dark:text-gray-400">Date de dernière connexion</span>
          <span className="text-gray-800 dark:text-gray-200">
          {formatDate(parseDate(currentUser.lastConnection))}
          </span>
        </li>
        <li className="flex flex-col">
          <span className="text-sm text-gray-500 dark:text-gray-400">Thème préféré</span>
          <span className="text-gray-800 dark:text-gray-200">
            {currentUser.prefTheme === "dark" ? "Sombre" : currentUser.prefTheme === "light" ? "Clair" : "Système"}
          </span>
        </li>
      </ul>
    </div>
