...
const ActivePixelBoards3DCarousel = () => {
    ...
    const fetchActivePixelBoards = async () => {
        try {
            ...
            const now = new Date();
            const activeBoards = response.data.data.filter(board => {
    ...
    return (
        <div className="space-y-6 relative">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h2 className="font-bold text-2xl mb-4 text-gray-800 dark:text-white">PixelBoards en cours</h2>
                ...
                {pixelBoards.length > 0 && !loading && (
                    <span className="px-3 py-1 bg-blue-100 text-blue-700 border border-blue-200 rounded-full text-sm font-medium">
                        {pixelBoards.length} PixelBoard{pixelBoards.length > 1 ? 's' : ''} actif{pixelBoards.length > 1 ? 's' : ''}
                    </span>
                )}
            </div>
            ...
            {pixelBoards.length === 0 && !loading ? (
                <div className="text-center py-12 bg-gray-50 border border-gray-200 rounded-lg shadow-sm">
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-700 mb-2 font-medium">Aucun PixelBoard actif</p>
                        <p className="text-sm text-gray-500">Revenez plus tard ou créez votre propre PixelBoard</p>
                    </div>
                </div>
            ) : (
            ...
            {loading && (
                <div className="flex flex-col items-center justify-center py-10">
                    ...
                    <p className="text-gray-500 text-sm mt-4">Chargement des PixelBoards...</p>
                </div>
            )}
