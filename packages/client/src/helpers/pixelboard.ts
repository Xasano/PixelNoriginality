...
class PixelBoardService {
    ...
    static async getActivePixelBoards(page = 1, limit = 10) {
        try {
            ...
            console.error("Erreur lors de la récupération des PixelBoards actifs:", error);
            throw error;
        }
    }
    ...
    static async getPixelBoardById(id) {
        try {
            ...
            console.error(`Erreur lors de la récupération du PixelBoard ${id}:`, error);
            throw error;
        }
    }

    // Vérifier si un PixelBoard est actif
    static isActiveBoard(board) {
    ...
    static filterActiveBoards(boards) {
