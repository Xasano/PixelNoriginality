    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6 relative">
        ...
        <p className="mb-4 text-gray-700 dark:text-gray-300">
          Êtes-vous sûr de vouloir supprimer le PixelBoard "{title}" ? Cette
...
const PixelBoardPreview: React.FC<{
  board: PixelBoardWithParticipantCount;
}> = ({ board }) => {
  return (
    <div
...
const PixelBoardList: React.FC<PixelBoardListProps> = ({
  ...
  const [pixelBoards, setPixelBoards] = useState<
    PixelBoardWithParticipantCount[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [pixelBoardToDelete, setPixelBoardToDelete] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchPixelBoards = async () => {
      try {
          ...
          (response.data.data || []).map(
            async (board: PixelBoardWithParticipantCount) => {
              try {
                ...
                console.error(
                  `Erreur lors de la récupération des détails pour ${board._id}:`,
                  err,
                );
        ...
        if (showOnlyActive) {
          const now = new Date();
          boardsWithDetails = boardsWithDetails.filter(
            (board) =>
        ...
        if (filters.startDate) {
          ...
          filteredBoards = filteredBoards.filter(
            (board) => new Date(board.creationDate) >= startDate,
          );
        }
        ...
        if (filters.endDate) {
          ...
          filteredBoards = filteredBoards.filter(
            (board) => new Date(board.creationDate) <= endDate,
          );
        }
        ...
        if (filters.minParticipants) {
          ...
          if (!isNaN(min)) {
            filteredBoards = filteredBoards.filter(
              (board) => (board.participantCount || 0) >= min,
            );
          }
        }
        ...
        if (filters.maxParticipants) {
          ...
          if (!isNaN(max)) {
            filteredBoards = filteredBoards.filter(
              (board) => (board.participantCount || 0) <= max,
            );
          }
        }
  ...
  const handleDeletePixelBoard = async () => {
    ...
    try {
      ...
      setPixelBoards((prev) =>
        prev.filter((board) => board._id !== pixelBoardToDelete),
      );
