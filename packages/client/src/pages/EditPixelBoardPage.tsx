...
import EditPixelBoardForm, { PixelBoardFormData } from "../components/EditPixelBoardForm";
...
const EditPixelBoardPage: React.FC = () => {
            ...
            try {
                ...
                if (response.data && response.data._id) {
                    ...
                    if (response.data.role === "admin") {
                        ...
                        const pixelBoardResponse = await axios.get(`http://localhost:8000/api/pixel-boards/${id}`, {
                            withCredentials: true,
                        });
                        ...
                        if (pixelBoardResponse.data.author._id === response.data._id) {
                            ...
                            navigate("/pixel-boards", {
                                state: { message: "Vous n'êtes pas autorisé à modifier ce PixelBoard" }
                            });
                        }
                    }
                } else {
                    ...
                    navigate("/login", {
                        state: { message: "Vous devez être connecté pour modifier un PixelBoard" }
                    });
                }
            } catch (err) {
    ...
    const handleEditPixelBoard = async (formData: PixelBoardFormData) => {
        try {
            const response = await axios.put(`http://localhost:8000/api/pixel-boards/${id}`, formData, {
                withCredentials: true,
                headers: {
            ...
            if (axios.isAxiosError(err) && err.response) {
                setError(err.response.data.message || "Erreur lors de la modification du PixelBoard");
