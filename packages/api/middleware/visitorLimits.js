...
export const limitVisitorRequests = (maxRequestsPerMinute = 30) => {
    return async (req, res, next) => {
        try {
            ...
            const checkResult = await req.visitor.checkRateLimit(maxRequestsPerMinute);
...
export const restrictVisitorAccess = (options = {}) => {
    return async (req, res, next) => {
        try {
            ...
            if (options.forbiddenActions && options.forbiddenActions.includes(req.method)) {
                throw new ApiErrorException(
                    ApiError.FORBIDDEN,
                    403,
            ...
            if (options.maxRequestsPerMinute) {
                const checkResult = await req.visitor.checkRateLimit(options.maxRequestsPerMinute);
